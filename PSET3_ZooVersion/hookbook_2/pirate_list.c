...
pirate *list_insert(pirate_list *pirates, pirate *p, size_t idx)
{
    ...
    */
    pirate **new_pirate_list = malloc((pirates->size + 1) * sizeof(pirate*));
    
    // copy pointers up to index into new pirate_list
    for (int i = 0; i < idx; i++) 
    {
        new_pirate_list[i] = pirates->pirates[i];
    }
    
    // Insert the new pirate 
    new_pirate_list[idx] = p;
    
    // copy in remaining pointers into new pirate list
    for (int i = idx; i < pirates->size; i++) 
    {
        new_pirate_list[i + 1] = pirates->pirates[i];
    }
    ...
    * of the program
    ...
    pirates->pirates = new_pirate_list;
    pirates->size++;
...
void list_contract_if_necessary(pirate_list* pirates)
{
    ...
    if(pirates->size == (pirates->capacity / RESIZE_FACTOR) && 
      (pirates->capacity / RESIZE_FACTOR) >= INITIAL_CAPACITY)
    {
