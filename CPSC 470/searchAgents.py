...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if corner in self.corners and not corner in corner_visited:
            corner_visited.append(corner)

        if len(corner_visited) == 4:
            return True
        return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                ...
                next = (nextx, nexty)
                if next in self.corners and not next in corners_visited_list:
                    corners_visited_list.append(next)
                successor = ((next, corners_visited_list), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    corners_visited = state[1]
    unvisited_corners_list = []
    for corner in corners:
        if not corner in corners_visited:
            unvisited_corners_list.append(corner)
        
    
    heuristic_list = [0]
    for corner in unvisited_corners_list:
...
def foodHeuristic(state, problem):
    ...
    food_layout = foodGrid.asList()

    heuristic_list = [0]
    for coordinate in food_layout:
        heuristic_list.append(manhattanDistance(position, coordinate))
