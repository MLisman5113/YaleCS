...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            self.beliefs[position] = self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition()) * self.beliefs[position]
    ...
    def elapseTime(self, gameState):
        ...
        for initial_ghost in self.allPositions:
            final_positions[initial_ghost] = self.getPositionDistribution(gameState, initial_ghost)

        for final_ghost in self.allPositions:
            probability = 0
            for initial_ghost in self.allPositions:
                probability += self.beliefs[initial_ghost] * final_positions[initial_ghost][final_ghost]
            beliefs[final_ghost] = probability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        total_positions = len(self.legalPositions)
        position_makeup = self.numParticles // total_positions
        
        for position in self.legalPositions:
            for i in range(position_makeup):
                self.particles.append(position)
        if self.numParticles % total_positions != 0:
            for i in range(self.numParticles % total_positions):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            list.append(self.getPositionDistribution(gameState, particle).sample())
        self.particles = list
