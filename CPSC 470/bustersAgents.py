...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for belief in livingGhostPositionDistributions:
            local_maximum.append(belief.argMax())
            
        for i, coordinate in enumerate(local_maximum):
            if goal_probability <= livingGhostPositionDistributions[i][coordinate]:
                goal_coordinate = coordinate
                goal_probability = livingGhostPositionDistributions[i][coordinate]

        for action in legal:
            list.append((self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), goal_coordinate), action))
