...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
            ...
            weight += (alpha * inActs[i] * delta)
            weights.append(weight)
            totalModification += abs(weight - previous_weight)
            i += 1
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for l in self.layers:
            new_act_list = []
            for node in l:
                new_act_list.append(node.sigmoidActivation(act_list))
            act_list = new_act_list
            list.append(new_act_list)
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in range(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2]) * (example[1][outputNum] - lastLayerOutput[outputNum])
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                hiddenDelta = []
                i = 0
                for neuronNum in self.layers[layerNum]:
                    weighted_sum = 0
                    j = 0
                    nextLayer = self.layers[layerNum+1]
                    for layer in nextLayer:
                        weighted_sum += deltas[0][j] * layer.weights[i+1]
                        j += 1
                    gPrime = neuronNum.sigmoidActivationDeriv(allLayerOutput[layerNum])
                    delta = weighted_sum * gPrime
            ...
            for numLayer in range(0,self.numLayers):
                ...
                for numNeuron in range(len(layer)):
                    weightMod = layer[numNeuron].updateWeights(allLayerOutput[index], alpha, deltas[index][perception_index])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for in_ex, out_ex in examplesTest:
        list = nnet.feedForward(in_ex)[-1]
        correct = True
        for val in range(len(list)):
            if round(list[val]) != out_ex[val]:
                correct = False
        if correct:
            testCorrect += 1
        else:
    ...
    testAccuracy= testCorrect / ((testError + testCorrect) * 1.0)
