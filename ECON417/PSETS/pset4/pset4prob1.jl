...
function valueFuncIter(U, a, F; beta=0.9, v0=zeros(length(a)), toler=1e-6, maxiter=1000, verbose=1)
    ...
    U_ar = [U(F(a[i])-F(a[j])) for i in 1:N, j in 1:N] # NxN array of all possible values of U, given grid a
    err = 1+toler; niter = 0;
...
function lifetime_utility(consumption, T, beta)
    value = 0
    for t in 1:T-1
        if consumption[t] == -Inf || consumption[t] == 0
            continue
        end
        value = value + ((beta^t)*log(consumption[t]))
    end
    return value
end
...
T = 1000
a_t = a[getPath(g, N, T)] # path of stock
println(a_t)
