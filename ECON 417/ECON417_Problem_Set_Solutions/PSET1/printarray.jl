...
# Prints out two-dimensional array as table with (optional) specified column names. Numbers in the jth column are formatted according to C-style formatting string formats[j]; can also specify single format string for all columns (default).
