...
(w, u) = gauher(12) # weights and abscissas to be used when calculating approximate expectation
b = 1; σ = 0.5; β = 0.7; r = 0.2; # model parameters
u2 = [3.636159118837738, 2.7327316742327897, 1.9566836492998819, 1.2366108297895136, 0.5429013272237046, -0.5429013272237046, -1.2366108297895136, -1.9566836492998819, -2.7327316742327897, -3.636159118837738]
# 
...
function G(t,a)
    result = 0
    for i=1:10
        result += w[i]*v[t]((1+r)*a+b+exp(σ*u2[i]*sqrt(2)))
    end
    result /= sqrt(π)
    return result
end
